...
const Banner = () => {
  ...
  return (
    <section className="h-max  space-y-[1px] font-merriwather text-white  bg-black overflow-hidden">
      <div
        onMouseEnter={handleVideo}
        onMouseLeave={handleVideo}
        className={`relative hover:scale-[1] scale-[0.992]  flex   w-full h-[400px]   transition-all ease-in-out    `}
      >
        <video
        ...
        <div className="absolute p-5 right-0 left-0 top-0 z-30 flex h-full w-full transition-all duration-200 hover:bg-black/50 ">
          <h1 className="text-[2rem] text-fondoBlanco md:text-[3rem] lg:text-[4rem] italic  ">
            Fotografías
          </h1>
        </div>
      </div>
      <div
        ...
        onMouseLeave={
          handleVideo1
        } /*en esta zona cambian la altura de la carta*/
        className={`relative hover:scale-[1] scale-[0.992]  flex   w-full h-[400px]   transition-all ease-in-out `}
      >
        <video
